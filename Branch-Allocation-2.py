...
fcuPowerConsumption = [7, 3, 10, 12, 4, 4, 18, 2, 5, 5, 5, 5]
numFcus = len(fcuPowerConsumption)
branchPowerBudget = 20
maxFcuPerBranch = 3
numBranches = 5   # Given the above listed FCU, total consumption 80, 5 branches are enough

# ------- Printing Program Data -------
print("Problem: allocate a set of FCU's with total power consumption of {} into branches of capacity {}, no more than {} FCU's per bramch.".format(
    sum(fcuPowerConsumption), branchPowerBudget, maxFcuPerBranch))
...
# Constraint 3: Each branch has a maximum number of FCU's
for j in range(numBranches):
    FcuMaxNumber = cqm.add_constraint(
        sum(FcuInBranch[i][j] for i in range(numFcus)) - branchUsed[j] * maxFcuPerBranch <= 0,
        label=f'max_fdus_branch_{j}')
